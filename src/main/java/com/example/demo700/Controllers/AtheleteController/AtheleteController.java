...
public class AtheleteController {
	...
	public ResponseEntity<?> searchAteleteByAge(@RequestParam String age, HttpServletRequest request) {
		return handleAutoPagination(request, "age", age, null);
	}

	@GetMapping("/searchByHeight")
	public ResponseEntity<?> searchAtheleteByHeight(@RequestParam String height, HttpServletRequest request) {
		return handleAutoPagination(request, "height", height, null);
	}

	@GetMapping("/findByTeamName")
	public ResponseEntity<?> findByTeamName(@RequestParam String teamName, HttpServletRequest request) {
		return handleAutoPagination(request, "team", teamName, null);
	}

	@GetMapping("/findByWeightLessThan")
	public ResponseEntity<?> findByWeightLessThan(@RequestParam String weight, HttpServletRequest request) {
		return handleAutoPagination(request, "weight", weight, null);
	}

	@GetMapping("/findByPosition")
	public ResponseEntity<?> findByPosition(@RequestParam String position, HttpServletRequest request) {
		return handleAutoPagination(request, "position", position, null);
	}

	@GetMapping("/findByEventAttendence")
	public ResponseEntity<?> findByEventAttendence(@RequestParam String eventName, HttpServletRequest request) {
		return handleAutoPagination(request, "event", eventName, null);
	}

	@GetMapping("/findByGameLog")
	public ResponseEntity<?> findByGameLog(@RequestParam String log, HttpServletRequest request) {
		return handleAutoPagination(request, "gameLog", log, null);
	}

	@GetMapping("/searchByPartialTeamName")
	public ResponseEntity<?> searchByPartialTeamName(@RequestParam String partialName, HttpServletRequest request) {
	...
	public ResponseEntity<?> findByAgeLessThanAndHeightGreaterThan(@RequestParam String height,
			@RequestParam String age, HttpServletRequest request) {
		return handleAutoPagination(request, "ageHeight", age + "," + height, null);
	}

	@PostMapping("/findByMultipleEvents")
	public ResponseEntity<?> findByMultipleEvents(@RequestBody List<String> eventNames, HttpServletRequest request) {
		return handleAutoPagination(request, "multipleEvents", String.join(",", eventNames), null);
	}

	@GetMapping("/findByNamePartial/{partialName}")
	public ResponseEntity<?> findByNamePartial(@PathVariable String partialName) {

		try {

			return ResponseEntity.status(200).body(atheleteService.findByNamePartial(partialName));
	...
	public ResponseEntity<?> findByGender(@PathVariable String gender, HttpServletRequest request) {

		try {

			return handleAutoPagination(request, "gender", gender, null);
		...
		try {
			...
			if (searchType == null || searchValue == null) {
				...
				case "multipleEvents":
					...
					break;
				case "gender":
					response = atheleteService.findByGender(searchValue, page, size);
					break;
				default:
					response = atheleteService.seeAll(page, size);
				}
	...
	private void addNavigationLinks(AthleteListResponseDTO response, HttpServletRequest request, String searchType,
		...
		if (currentPage < totalPages - 1) {
			response.setNextLink("/api/athelete/next?currentUrl=" + response.getSelfLink() + "&searchType="
					+ (searchType != null ? searchType : "") + "&searchValue="
					+ (searchValue != null ? searchValue : ""));
		}

		if (currentPage > 0) {
			response.setPrevLink("/api/athelete/prev?currentUrl=" + response.getSelfLink() + "&searchType="
					+ (searchType != null ? searchType : "") + "&searchValue="
					+ (searchValue != null ? searchValue : ""));
		}
	...
	public ResponseEntity<?> updateAthelete(@RequestParam String id, @RequestBody Athelete updatedAthelete,
